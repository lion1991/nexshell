...
| （新，真机发现） | nix rdpdr 后端对 `LockControl` IRP 回 STATUS_SUCCESS（同 FreeRDP，不真正加锁） | Win11 往共享盘拷文件卡在 99%：Explorer 写完 `:Zone.Identifier` 流后发共享锁请求，旧 TODO 不应答，服务端一直等 |
...
- IronRDP：fork rev `d9ee67594adf02634805ae5f2d4b4db42785e8a6`（分支 `nexshell-2026-08`），上游合并基 `872845c`。
