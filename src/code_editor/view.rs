...
use warp_editor::render::model::{
    AutoScrollMode, BlockSpacing, BrokenLinkStyle, CheckBoxStyle, Decoration, ExpansionType,
...
pub trait ShowFindReferencesCardProvider: std::fmt::Debug + 'static {
    fn should_show_find_references_card(
        &self,
        card_anchor_location: pathfinder_geometry::rect::RectF,
        app: &AppContext,
    ) -> bool;
}
...
/// 本地化自 warp::notebooks::editor::rich_text_styles —— code_text_styles 的基底；
/// table_style 内联（绕开 markdown 依赖），代码渲染不触发表格。
fn rich_text_styles(appearance: &Appearance, font_settings: &FontSettings) -> RichTextStyles {
