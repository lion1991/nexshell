...
use warp::ai::blocklist::InputType;
use warp::appearance::Appearance;
use crate::text_editor::EditorElement;
use warp::server::server_api::TranscribeError;
use warp::server::telemetry::TelemetryEvent;
use warp::settings::{AISettings, VoiceInputToggleKey};
use warp::themes::theme::Fill;
use warp::ui_components::buttons::{icon_button, icon_button_with_color};
use warp::ui_components::icons;
use warp::view_components::{FeaturePopup, NewFeaturePopupLabel};
use warp::workspace::ToastStack;
...
impl EditorView {
    ...
    pub(super) fn create_voice_new_feature_popup(
        ...
        ctx.subscribe_to_view(&voice_new_feature_popup, |_me, _, event, ctx| {
            if matches!(
                event,
                warp::view_components::NewFeaturePopupEvent::Dismissed
            ) {
    ...
    fn voice_error_toast(&mut self, message: &str, ctx: &mut ViewContext<Self>) {
        ...
        ToastStack::handle(ctx).update(ctx, |toast_stack, ctx| {
            let toast = warp::view_components::DismissibleToast::error(message.to_string());
    ...
    pub fn toggle_voice_input(
        ...
        match &self.voice_input_state {
            VoiceInputState::Stopped => {
                ...
                if !warp::ai::AIRequestUsageModel::handle(ctx)
                    .as_ref(ctx)
                    .can_request_voice()
                {
                ...
                if self.focused || matches!(*source, voice_input::VoiceInputToggledFrom::Button) {
                    ...
                    let is_udi_enabled = warp::settings::InputSettings::handle(ctx)
                        .as_ref(ctx)
                    ...
                    if matches!(*source, voice_input::VoiceInputToggledFrom::Button) {
                        ...
                        AISettings::handle(ctx).update(ctx, |settings, ctx| {
                            if let Some(toggle_key) = settings.maybe_setup_first_time_voice(ctx) {
                                ToastStack::handle(ctx).update(ctx, |toast_stack, ctx| {
                                    let toast = warp::view_components::DismissibleToast::success(
                                        format!(
    ...
    fn show_microphone_access_toast(ctx: &mut ViewContext<Self>) {
        ...
        ToastStack::handle(ctx).update(ctx, move |toast_stack, ctx| {
            let mut toast = warp::view_components::DismissibleToast::error(String::from(
    ...
    fn render_voice_transcription_button_tooltip(
        &self,
        appearance: &warp::appearance::Appearance,
        app: &AppContext,
